...
local function create_combinator(logi_slot, logi_item, logi_min, logi_max)
  local result = {
    name = "constant-combinator",
    position = {
      -- Conceptually, we consider slot #1 to be (0,0), but the game has
      -- combinators align on 0.5 because the coordinate is their center,
      -- the middle of a tile. So in the blueprint, it will be (0.5,0.5).
      -- All other coordinates are relative to it.
      x = ((logi_slot - 1) % 10) + 0.5,
      y = floor((logi_slot - 1) / 10) + 0.5
    },
    control_behavior = {
      filters = {
        [1] = {
          count = logi_min, -- slot.min encoded here
          index = 1,
          signal = {
            name = logi_item, -- slot.name encoded here
            type = "item"
          }
        }
      }
  ...
  if logi_max < LOGISTICS_DEFAULT_MAX then
    result.control_behavior.filters[2] = {
      count = logi_max, -- slot.max encoded here
      index = 2,
      signal = {
        name = "signal-dot",
        type = "virtual"
...
local function create_dummy_combinator()
  local result = {
    name = "constant-combinator",
    position = {
      -- The entire reason we exist. Dummy combinators have top-left
      -- coordinates, because something has to.
      x = 0.5,
      y = 0.5
    },
    control_behavior = {
...
local function export_to_blueprint(player)
  ...
  -- Slot 1 is special. Most empty slots are skipped, but Slot 1 can't be
  -- skipped because all blueprint coordinates are relative to it. So it gets a
  -- dummy value.
  local slot1 = player.get_personal_logistic_slot(1)
  if not slot1.name then
    local new_combinator = create_dummy_combinator()
    n_found = n_found + 1
    new_combinator.entity_number = n_found
    blueprint_entities[n_found] = new_combinator
  end

  for i = 1,n_logi do
    local slot = player.get_personal_logistic_slot(i)
    if slot.name then
      local new_combinator = create_combinator(i, slot.name, slot.min, slot.max)
      n_found = n_found + 1
      new_combinator.entity_number = n_found
      blueprint_entities[n_found] = new_combinator
    end
